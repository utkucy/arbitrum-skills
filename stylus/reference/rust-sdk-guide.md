...
```rust
fn mistake(vec: &mut StorageVec) -> U64 {
...
```rust
fn nested_vec(vec: &mut StorageVec>) {
    let mut inner = vec.grow();  // adds a new element accessible via `inner`
    inner.push(0.into());        // inner is a guard to a StorageVec
}

fn nested_map(map: &mut StorageMap<u32, StorageVec>) {
...
impl Contract {
    ...
    #[fallback]
    pub fn fallback(&mut self, calldata: Vec<u8>) -> Result, Vec<u8>> {
...
```rust
pub fn do_call(&mut self, account: IService, user: Address) -> Result {
