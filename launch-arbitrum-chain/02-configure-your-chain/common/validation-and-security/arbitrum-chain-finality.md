...
Generally, transactions executed through the sequencer on Arbitrum chains [achieve finality](/how-arbitrum-works/deep-dives/transaction-lifecycle.mdx) equivalent to their parent chain once the relevant transaction data has been [posted in a batch](/how-arbitrum-works/01-inside-arbitrum-nitro.mdx). This means that transactions on Arbitrum chains are considered final in minutes.
...
Additionally, the delay in L3 finalization can be decreased to achieve extremely fast (one minute) deposits by configuring the sequencer to wait for fewer L2 block confirmations:
