...
```shell
cast send  "setExtraChallengeTimeBlocks(uint256)" 300 \
  --rpc-url  \
  --private-key 
```
...
`` with the contract address of the Rollup admin.
`` with the appropriate RPC endpoint.
`` with the private key of the authorized admin account (e.g., chain owner).
