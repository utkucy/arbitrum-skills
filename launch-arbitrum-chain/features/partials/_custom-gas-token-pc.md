...
- **Integration with existing tools**: Native implementation in Arbitrum ensures compatibility with EVM tooling, deterministic gas accounting, and seamless deployment.
...
- **Requirements and limitations**: The token must meet specific criteria (e.g., standard `ERC-20`). While supported in, it relies on chain-wide adoption, which may not be suitable for all cases.