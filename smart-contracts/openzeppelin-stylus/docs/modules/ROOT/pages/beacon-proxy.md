...
* [Basic proxy](proxy.md): Basic proxy pattern using `delegate_call` for upgradeable contracts.
* [Beacon Proxy](beacon-proxy.md): Multiple proxies pointing to a single beacon contract for mass upgrades of the implementation contract address.
* [UUPS Proxy](uups-proxy.md): The Universal Upgradeable Proxy Standard (UUPS) that is a minimal and gas-efficient pattern for upgradeable contracts.